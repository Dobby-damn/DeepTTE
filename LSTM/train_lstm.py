...
class LSTMClassifier(nn.Module):
    ...
    def __init__(
        self,       
        input_size: int = 22,  # 2 (ex, ey) + 20 (属性特征)
        hidden_size: int = 128,
        num_layers: int = 10,
        num_classes: int = 2,
        dropout: float = 0.2,
        ...
        self.lstm = nn.LSTM(
            ...
            bidirectional = self.bidirectional,  # 双向结构
            batch_first = True,
        ...
        self.attn = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_size, 1, bias = False)  # attention weights
        )
...
def main():  # noqa: D401
    ...
    learning_rate = 1e-4
    dropout = 0.2
    input_size = 22  # 2 (ex, ey) + 20 (属性特征)
    num_layers = 10 #LSTM 层数
