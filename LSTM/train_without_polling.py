...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, x, lengths, static_feats):
        ...
        # (batch, seq, 2*hidden)
        # ----- 4. last-timestep embedding
        # last_idx = lengths - 1   # 每个样本的最后有效位置
        last_idx = (lengths - 1).view(-1, 1, 1).expand(-1, 1, lstm_out.size(-1))  # (B, 1, 256)
        seq_repr = lstm_out.gather(1, last_idx).squeeze(1)  # (batch, 256)
        # ----- 5. 静态特征 MLP -----
