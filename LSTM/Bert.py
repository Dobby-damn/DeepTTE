...
class BertForTrajectory(nn.Module):
    def __init__(self, feature_dim, hidden_size=128, num_hidden_layers=6, num_attention_heads=8, output_dim=1, dropout=0.1):
        super(BertForTrajectory, self).__init__()
        # 定义BERT配置
        config = BertConfig(
            ...
            vocab_size=1,  # 不用词表，这里只是占位
            max_position_embeddings=512,
            type_vocab_size=1,
            pad_token_id=0
        )
        # 嵌入层，将数值特征映射到hidden_size
    ...
    def forward(self, x, lengths):
        ...
        embeds = self.feature_embedding(x)  # (batch, seq_len, hidden_size)

        # attention mask: 1 表示保留, 0 表示padding
        batch_size, seq_len, _ = embeds.size()
        attention_mask = torch.zeros((batch_size, seq_len), dtype=torch.long, device=x.device)
        for i, l in enumerate(lengths):
            attention_mask[i, :l] = 1

        outputs = self.bert(inputs_embeds=embeds, attention_mask=attention_mask)
...
def eval_epoch(model, loader, criterion, device):
    ...
    return  avg_loss, accuracy,f1, f1_class0  # 正常类（类别0）的F1
...
def main():  # noqa: D401
    ...
    model = BertForTrajectory(feature_dim=2, hidden_size=128, num_hidden_layers=6, num_attention_heads=8, output_dim=1).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        val_loss, val_acc, f1, f1_class0 = eval_epoch(model, val_loader, criterion, device)
