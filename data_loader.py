...
if __name__ == "__main__":
    ...
    test_data = [
        # 样本1 (长度3)
        {
            'dist': 10.5,                    # 总距离(km)               可计算
            'time': 3600,                     # 总时间(秒)              可计算
            'driverID': 1,                    # 司机ID                  用户id evaluation_id
            'dateID': 101,                    # 日期ID                  无
            'weekID': 3,                      # 周ID                    无
            'timeID': 1,                      # 时间段ID                无
            'lngs': [116.404, 116.405, 116.406],  # 经度序列            ex
            'lats': [39.915, 39.916, 39.917],      # 纬度序列           ey
    ...
    print(traj)   #轨迹数据字典（填充后的序列和原始长度）
    print(1)
    print(attr)  #静态属性字典（标准化后的数值特征,没有类别id）
    test_data = [
        #样本1
        {
            'dist': 10.5,                    # 总距离            
