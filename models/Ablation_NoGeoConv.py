...
class Net(nn.Module):
    ...
    def __init__(self, 
        ...
        # 没有GeoConv的时空特征模块（SpatioTemporal_NoGeoConv.Net）
        self.spatio_temporal = base.SpatioTemporal_NoGeoConv.SpatioTemporalNet_NoGeoConv(
            attr_size=self.attr_net.out_size(),
            traj_input_dim=4,  # 原始轨迹特征维度
    ...
    def forward(self, attr, traj, config=None):
        ...
        # 提取时空轨迹特征
        sptm_vec = self.spatio_temporal(traj, attr_vec, config)  # (B, D_traj)
        # --- 改进的融合策略 ---
        # 简单的 Concat
