...
class LSTMClassifier(nn.Module):
    def __init__(
        ...
        self.seq_embed = nn.Linear(input_size, hidden_size * 2)  # 9 -> 256
        # Attention 层
    ...
    def forward(self, x, lengths, static_feats):
        ...
        # )  # (batch, seq, 2*hidden)
        seq_emb = self.seq_embed(x)   # (batch, seq, 256)
        # ----- 3. Multi-Head Attention -----
        # Q = K = V = lstm_out
        attn_out, _ = self.attn(seq_emb, seq_emb, seq_emb)  # (batch, seq, 256)
